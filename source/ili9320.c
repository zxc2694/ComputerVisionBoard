...
// second release on 3/5  ,luminance is acceptable,water wave appear during camera preview
  ...
  //Panel Interface Control//
	...
	{
		LCD_WriteReg(0x0000,0x0001);    ili9320_Delay(50000);  //打開晶振
    	...
    	LCD_WriteReg(0x0001,0x2B3F);    ili9320_Delay(50000);   //驅動輸出控制320*240  0x6B3F
    	LCD_WriteReg(0x0002,0x0600);    ili9320_Delay(50000);
    	LCD_WriteReg(0x0010,0x0000);    ili9320_Delay(50000);
    	LCD_WriteReg(0x0011,0x6070);    ili9320_Delay(50000);        //0x4030           //定義數據格式  16位色 		橫屏 0x6058
    	...
    	LCD_WriteReg(0x000F,0x0000);    ili9320_Delay(50000);        //掃瞄開始地址
...
/****************************************************************************
* 名    稱：void ili9320_SetCursor(u16 x,u16 y)
* 功    能：設置屏幕座標
* 入口參數：x      行座標
*           y      列座標
* 出口參數：無
* 說    明：
* 調用方法：ili9320_SetCursor(10,10);
****************************************************************************/
...
/****************************************************************************
* 名    稱：void ili9320_SetWindows(u16 StartX,u16 StartY,u16 EndX,u16 EndY)
* 功    能：設置窗口區域
* 入口參數：StartX     行起始座標
*           StartY     列起始座標
*           EndX       行結束座標
...
* 出口參數：無
* 說    明：
* 調用方法：ili9320_SetWindows(0,0,100,100)；
****************************************************************************/
...
/****************************************************************************
* 名    稱：void ili9320_Clear(u16 dat)
* 功    能：將屏幕填充成指定的顏色，如清屏，則填充 0xffff
* 入口參數：dat      填充值
* 出口參數：無
* 說    明：
* 調用方法：ili9320_Clear(0xffff);
****************************************************************************/
...
/****************************************************************************
* 名    稱：u16 ili9320_GetPoint(u16 x,u16 y)
* 功    能：獲取指定座標的顏色值
* 入口參數：x      行座標
*           y      列座標
* 出口參數：當前座標顏色值
* 說    明：
* 調用方法：i=ili9320_GetPoint(10,10);
****************************************************************************/
...
/****************************************************************************
* 名    稱：void ili9320_SetPoint(u16 x,u16 y,u16 point)
* 功    能：在指定座標畫點
* 入口參數：x      行座標
*           y      列座標
*           point  點的顏色
* 出口參數：無
* 說    明：
* 調用方法：ili9320_SetPoint(10,10,0x0fe0);
****************************************************************************/
...
/****************************************************************************
* 名    稱：void ili9320_DrawPicture(u16 StartX,u16 StartY,u16 EndX,u16 EndY,u16 *pic)
* 功    能：在指定座標範圍顯示一副圖片
* 入口參數：StartX     行起始座標
*           StartY     列起始座標
*           EndX       行結束座標
*           EndY       列結束座標
            pic        圖片頭指針
* 出口參數：無
* 說    明：圖片取模格式為水平掃瞄，16位顏色模式
* 調用方法：ili9320_DrawPicture(0,0,100,100,(u16*)demo);
****************************************************************************/
...
/****************************************************************************
* 名    稱：void ili9320_PutChar(u16 x,u16 y,u8 c,u16 charColor,u16 bkColor)
* 功    能：在指定座標顯示一個8x16點陣的ascii字符
* 入口參數：x          行座標
*           y          列座標
*           charColor  字符的顏色
*           bkColor    字符背景顏色
* 出口參數：無
* 說    明：顯示範圍限定為可顯示的ascii碼
* 調用方法：ili9320_PutChar(10,10,'a',0x0000,0xffff);
****************************************************************************/
  ...
  for (i=0;i<16;i++)
  {
    ...
    for (j=0;j<8;j++)
    {
      if ( (tmp_char >> 7-j) & 0x01 == 0x01)
        {
          ili9320_SetPoint(x+j,y+i,charColor); // 字符顏色
        }
        else
        {
          ili9320_SetPoint(x+j,y+i,bkColor); // 背景顏色
        }
    }
  }
}
...
/****************************************************************************
* 名    稱：u16 ili9320_BGR2RGB(u16 c)
* 功    能：RRRRRGGGGGGBBBBB 改為 BBBBBGGGGGGRRRRR 格式
* 入口參數：c      BRG 顏色值
* 出口參數：RGB 顏色值
* 說    明：內部函數調用
* 調用方法：
...
/****************************************************************************
* 名    稱：void ili9320_BackLight(u8 status)
* 功    能：開、關液晶背光
* 入口參數：status     1:背光開  0:背光關
* 出口參數：無
* 說    明：
* 調用方法：ili9320_BackLight(1);
****************************************************************************/
...
/****************************************************************************
* 名    稱：void ili9320_Delay(vu32 nCount)
* 功    能：延時
* 入口參數：nCount   延時值
* 出口參數：無
* 說    明：
* 調用方法：ili9320_Delay(10000);
****************************************************************************/
